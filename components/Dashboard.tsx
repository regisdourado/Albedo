        ...
        <div className="space-y-6 animate-in fade-in zoom-in-95 duration-500">
           <div className="relative w-full h-[700px] bg-slate-950 rounded-xl border border-slate-700 overflow-hidden group select-none shadow-2xl" onClick={() => setSelectedRegion(null)}>
              ...
              {!isLoading && (
                <>
                  <button 
                    ...
                    className={`absolute top-4 right-4 z-40 p-3 rounded-full border shadow-2xl transition-all hover:scale-110 active:scale-95 ${isFilterMenuOpen ? 'bg-orange-600 border-orange-400 text-white' : 'bg-slate-900/90 border-slate-700 text-slate-400 hover:text-white backdrop-blur-md'}`}
                  >
                    <Filter size={20} />
                  </button>
                  {isFilterMenuOpen && (
                    <div ref={filterMenuRef} className="absolute top-16 right-4 bg-slate-900/95 backdrop-blur-xl p-5 rounded-xl border border-slate-700 w-64 shadow-2xl z-50 animate-in fade-in slide-in-from-top-4 duration-300" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="w-full h-full relative transition-transform duration-700" style={{ transform: selectedRegion ? 'scale(1.8)' : 'scale(1)', transformOrigin: selectedRegion ? `${selectedRegion.x} ${selectedRegion.y}` : 'center center' }}>
                  {!isLoading && riskRegions.map((region) => {
                      const isActive = activeRiskFilters.includes(region.riskLevel);
                      const regionColor = getRiskColorHex(region.riskLevel);
                      return (
                        <div key={region.id} className={`absolute transition-all duration-500 ${isActive ? 'scale-100 opacity-100' : 'scale-0 opacity-0 pointer-events-none'}`} style={{ top: region.y, left: region.x, width: '0', height: '0', zIndex: hoveredRegion === region.id ? 50 : 10 }} onClick={(e) => { e.stopPropagation(); setSelectedRegion(region); }} onMouseEnter={() => !selectedRegion && setHoveredRegion(region.id)} onMouseLeave={() => setHoveredRegion(null)}>
                          ...
                          <div className="relative z-20 flex flex-col items-center"><MapPin size={24} fill={regionColor} className="text-slate-900 stroke-[1.5px] transition-transform hover:scale-125" /></div>
                          {hoveredRegion === region.id && !selectedRegion && (
                            <div className="absolute z-[100] w-64 pointer-events-none animate-in fade-in slide-in-from-bottom-2 duration-200 bottom-full pb-4 -translate-x-1/2">
                              <div className="bg-slate-900/95 backdrop-blur-xl border border-slate-600/50 rounded-lg shadow-2xl p-4 text-left">
                                <div className="flex items-center gap-2 mb-3 border-b border-slate-800 pb-2"><Navigation size={12} className="text-orange-500" /><span className="text-xs font-bold text-white font-mono uppercase truncate">{region.name}</span></div>
                                <div className="space-y-2"><div className="flex justify-between items-end"><span className="text-[10px] text-slate-500 uppercase font-bold tracking-wider">Temperatura</span><span className="text-xl font-bold text-white leading-none">{region.temperature}°C</span></div><div className="flex justify-between items-center pt-2 border-t border-slate-800/50"><span className="text-[10px] text-slate-500 uppercase font-bold tracking-wider">Nível de Risco</span><span className="text-[10px] font-bold px-2 py-0.5 rounded border uppercase" style={{ backgroundColor: `${regionColor}20`, color: regionColor, borderColor: `${regionColor}40` }}>{region.riskLevel}</span></div></div>
                              </div>
                            </div>
                          )}
                        </div>
