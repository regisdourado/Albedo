                  ...
                  <div ref={filterMenuRef} className="absolute top-16 right-4 bg-slate-900/95 backdrop-blur-xl p-5 rounded-xl border border-slate-700 w-64 shadow-2xl z-[500] animate-in fade-in slide-in-from-top-4 duration-300" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="space-y-3">
                      {['Crítico', 'Alto', 'Médio', 'Baixo'].map((level) => {
                        ...
                        return (
                          <button key={level} onClick={() => toggleRiskFilter(level)} className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg transition-all text-sm border-2 group ${isActive ? 'text-white font-bold shadow-inner' : 'border-transparent text-slate-500 opacity-40 hover:opacity-100 hover:bg-slate-800/50'}`} style={{borderColor: isActive ? riskColor : 'transparent', backgroundColor: isActive ? `${riskColor}15` : 'rgba(30, 41, 59, 0.3)'}}>
